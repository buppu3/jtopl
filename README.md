...
OPL3 相当モジュールです。
cen ポートの入力には 14.32MHz のクロックイネーブル信号が必要です。

## jtopl8960.v

OPL3 / dual-OPL2 モジュールです。
cen ポートの入力には 14.32MHz のクロックイネーブル信号が必要です。

レジスタ 105h の NEW ビットで OPL3 / dual-OPL2 を切り替えます。

NEW ビットが 1 の時、OPL3 相当で動作します。
NEW ビットが 0 の時は OPL2 相当の音源が 2つ使えます(リズム音、タイマー共に2つ使えます)。

OPL2 モード動作時は chA に OPL2-0、chB に OPL2-1 のサウンド信号が出力されます。
OPL3 モード時はレジスタの設定に従いサウンド信号を出力します。
